...
const Products = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/products2") // replace with your JSON Server URL
      .then((res) => res.json())
      ...
      <div className="grid grid-cols-1 cursor-pointer sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 justify-center px-4">
        {products.map((item, index) => (
          <Link to={`/ProductDetail/${item.id}`} key={index}>
            <div className="relative group overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
